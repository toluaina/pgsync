        ...
        for key in path.split('/'):
            if value:
                if isinstance(value, list):
                    value = [
                        v.get(key, default) if v else None for v in value
                    ]
                else:
...
class TypeAheadView(web.View):
    ...
    def build_queries(self, mapping, search_param):
        ...
        for key, value in mapping.items():
            ...
            if isinstance(value, dict):
                queries.extend(
                    self._build_queries(key, value, search_param)
                )
        return queries
    ...
    def build_highlight(self, mapping):
        ...
        for key, value in mapping.items():
            ...
            if isinstance(value, dict):
                highlights.extend(
                    self._build_highlight(key, value)
                )
        return highlights
